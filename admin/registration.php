...
<br>
				<h1 style="text-align: center;font-size: 35px; color: pink; font-weight: bolder;">Admin Registration</h1><br>
				<h4 style="color:yellow; margin-left:175px; margin-top:-10px;"><b>(Only for the Library Staff)</b></h4>
				<form name="Registration" action="" method="post">
					
					<div class="login">
						

						Staff ID<input class="form-control" type="text" name="staff" placeholder="Enter your registered staff ID number" required=""><br>
	...
	<?php
		...
		{
			$count=0;
			$sql="select staffID from `login`";
			$res=mysqli_query($db,$sql);

			while($row=mysqli_fetch_assoc($res))
			{
				if($row['staffID']==$_POST['staff'])
				{
					$count=$count+1;
				}
			}
			...
			if($count==0)
			{
				mysqli_query($db,"insert into `login` (userType, staffID, firstname, lastname, birthday, email, nic, phoneNo, address, registeredDate, username, password,image )values('admin'
					,'$_POST[staff]','$_POST[firstname]' ,'$_POST[lastname]' ,'$_POST[birthday]' ,'$_POST[email]' ,'$_POST[nic]' ,'$_POST[phoneNo]' ,'$_POST[address]' ,'$_POST[registeredDate]','$_POST[username]' ,'$_POST[password]','user.png');");
	...
	else
	{
		?>
		<script type="text/javascript">
			alert("Sorry, someone has already registered with this Staff ID to our system !  Not you? Please contact us.");
		</script>
	<?php
	}
}
